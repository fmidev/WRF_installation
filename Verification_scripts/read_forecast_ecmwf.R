####################################
# Read ECMWF forecast and save to SQLite #
####################################

# Parse command line arguments - expecting forecast date in format yyyymmddHH
args = commandArgs(trailingOnly=TRUE)
if (length(args) != 1 || nchar(args[1]) != 10){
  stop("Usage: Rscript read_forecast_ecmwf.R <yyyymmddHH>\n  Example: Rscript read_forecast_ecmwf.R 2024010100")
}
datetime = args[1]
...
inspect_grib <- function(file_path) {
  tryCatch({
    ...
    system(paste("grib_ls -p shortName,name,level", file_path, "| head -30"), ignore.stdout = FALSE)
    return(TRUE)
  }, error = function(e) {
...
station_list <- read.csv("/wrf/wrf-data/WRF_Model/Verification/Data/Static/stationlist.csv")
file_path <- "/wrf/wrf-data/WRF_Model/Verification/Data/Forecast" 
template <- "{fcst_model}_{YYYY}{MM}{DD}{HH}"
sql_folder <- "/wrf/wrf-data/WRF_Model/Verification/SQlite_tables/FCtables"
fcst_model <- "ecmwf"
forecast_file <- paste0(file_path, "/", fcst_model, "_", datetime)

# Define ECMWF variable names and their mapping
ecmwf_vars <- c("t2m", "ws10m", "q2m", "psfc", "topo", "pcp")

cat("Processing ECMWF forecast for date:", datetime, "\n")
...
tryCatch({
  ...
  cat("- Variables requested:", paste(ecmwf_vars, collapse=", "), "\n")
  cat("- ECMWF uses: 2t (temp), 2sh (specific humidity from dewpoint), 10u/10v (wind), sp (pressure), tp (precip), orog (topography)\n")
}) 
 
cat("ECMWF forecast processing completed\n")