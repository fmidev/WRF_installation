...
if (length(args) != 1){
  stop("Give forecast date/time as input argument in yyyymmddHH format, E.g. Rscript read_forecast_wrf_d01.R 2024010100")
}
...
cat("Using model: wrf_d01\n")

# Check if forecast file exists
forecast_file <- paste0(file_path, "/wrf_d01_", datetime)
...
cat("Forecast processing completed for wrf_d01\n")

if (!file.exists(forecast_file)) {
  cat("ERROR: Forecast file not found:", forecast_file, "\n")
  # Look for files in the directory
  cat("Files in directory", file_path, ":\n")
  dir_files <- list.files(file_path, pattern = "wrf_d01_")
  print(dir_files)
  stop(paste0("Forecast file not found: ", forecast_file))
}
