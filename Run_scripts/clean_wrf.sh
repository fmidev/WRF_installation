...
if [ -d "$TEST_BASE_DIR" ]; then
    find $TEST_BASE_DIR/DA_input/rc -type f -ctime +1 -name "*wrfout*" 2>/dev/null | xargs -r rm
    find $TEST_BASE_DIR/DA_input/ob/raw_obs -type f -ctime +1 2>/dev/null | xargs -r rm
    find $TEST_BASE_DIR/out -type f -ctime +2 -o -type l -ctime +2 2>/dev/null | xargs -r rm
    find $TEST_BASE_DIR/out -mindepth 1 -type d -empty 2>/dev/null -delete
fi